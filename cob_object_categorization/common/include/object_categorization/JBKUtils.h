/// @file JBKUtils.h
/// Basic utilities from JBK. These include double and integer vectors and matrices,
...
public:
	...
	void GetStatistics(double& Mean, double& Sigma) const;  
...
public:
	...
	/// Yields the cluster labels.
	double IsInCluster(const DblVector& X, double thresh, unsigned int cluster); 
	
	/// Add to clusters.
	int AddToClusters(const DblVector& X, double thresh);

	/// Retrieve cluster num.
	int RetrieveClusterNum(const DblVector& X, double thresh);//, IntVector* filter);

	/// QT clustering
	int qtClustering(double thresh, std::vector<DblMatrix>& clusters);
	int qtClusteringThis(double thresh);
};
...
inline int removeFile(std::string& Name) { return remove(Name.c_str()); }

} // end namespace ipa.utils
