/// @file JBKUtils.cpp
/// Implementations of the classes/functions declared and documented in JBKUtils.h
...
/// This line is necessary to prevent linker errors with tmplates
...
double DblMatrix::IsInCluster(const DblVector& X, double thresh, unsigned int cluster)
{
	if(cluster>=size()) return false;
	double thrsqr = thresh * thresh;
	double sum=0, min=0, minDist = DBL_MAX;
	for(unsigned int j=0; j<(*this)[cluster].size(); j++)
	{
		sum+=((*this)[cluster][j]-X[j])*((*this)[cluster][j]-X[j]);
		if(sum>thrsqr)
			return -1;
	}
	return sqrt(sum);
...
int DblMatrix::AddToClusters(const DblVector& X, double thresh)
{
	bool flag = false;
	for(unsigned int i=0; i<size(); i++)
	{
		if(IsInCluster(X, thresh, i))
		{
			return i;
		}
	}
...
int DblMatrix::RetrieveClusterNum(const DblVector& X, double thresh)//, IntVector* filter)
{
	double minDist = DBL_MAX, dist; int minCls = -1;
	for(unsigned int i=0; i<size(); i++)
	{
		if((dist=IsInCluster(X, thresh, i))!=-1)
		{
			if(dist<minDist)
			{
				minDist = dist;
...
int DblMatrix::qtClusteringThis(double thresh)
{
	/// initialize the members
	IntMatrix members;
	members.assign(size(), IntVector());
	...
	for(unsigned int i=0; i<members.size(); i++)
	{
		tmp.push_back((*this)[members[i][0]]);
	}
	*this = tmp;
...
void SaveHistogramAsPSTricksList(std::map<int, int>& Hist, const std::string& Name)
{
	std::stringstream s;
