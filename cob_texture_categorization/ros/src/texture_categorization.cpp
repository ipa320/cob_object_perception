...
//	trainingdata.compute_data(&path_traindata, 1, &path_save_location, 1135);
	...
	create_train_data database_data = create_train_data();									// Berechnet den Featurevektor und den einen Labelvektor zum Trainieren
	database_data.compute_data(&path_database, 0, &path_save_location, 1281);

	std::cout << "finished feature computation\n";
	getchar();
