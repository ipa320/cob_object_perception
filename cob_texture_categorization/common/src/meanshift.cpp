...
int MeanShift(const IplImage* img, int **labels, cv::Mat* depth_mat)
{
//	for(int i=0;i<100;i++)
//	{
//		for(int j=0;j<100;j++)
//		{
//			std::cout<<(*depth_mat).at<float>(i,j)<<"  ";
//		}
//	}
	std::cout<<"Ohne 3D--------------";
...
//	cvShowImage("tobeMeanShift",tobeshow);
...
//	cvReleaseImage(&tobeshow);
	...
	{
		int *modePointCounts_buffer = new int[regionCount];
		float *mode_buffer = new float[regionCount*3];							// Erweiterung 3 auf 4
		int	*label_buffer = new int [regionCount];
		int minRegionCount;
		...
		do{
			...
			if(modePointCounts[i] < minRegion)
			{
				...
				float minDistance = color_distance(&mode[3*i], &mode[3*candidate]);				// Erweiterung: von 3 auf 4
				neighbor = neighbor->next;
				while(neighbor)
				{
					float minDistance2 = color_distance(&mode[3*i], &mode[4*neighbor->label]);	// Erweiterung: von 3 auf 4
